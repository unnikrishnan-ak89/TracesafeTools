...
public class CommonUtils {
	...
	public static TagContactPacket getTagContactPacketData(InputStream csvDataStream) {
		InputStreamReader streamReader = new InputStreamReader(csvDataStream, StandardCharsets.UTF_8);
		...
		try (BufferedReader reader = new BufferedReader(streamReader)) {
			for (String line; (line = reader.readLine()) != null;) {
				...
				try {
					...
					int sequenceNumber = StringUtils.isNotBlank(arr[0]) ? Integer.parseInt(arr[0].trim()) : 0;
					int protocolVersion = StringUtils.isNotBlank(arr[1]) ? Integer.parseInt(arr[1].trim()) : 0;
					long beaconLoggerTick = StringUtils.isNotBlank(arr[2]) ? Long.parseLong(arr[2].trim()) : 0;
					long extractTime = StringUtils.isNotBlank(arr[3]) ? Long.parseLong(arr[3].trim()) : 0;
					int battery = Integer.parseInt(arr[4].trim());
					int guidIndex = StringUtils.isNotBlank(arr[5]) ? Integer.parseInt(arr[5].trim()) : 0;
					int count = StringUtils.isNotBlank(arr[6]) ? Integer.parseInt(arr[6].trim()) : 0;
					int rssi = StringUtils.isNotBlank(arr[7]) ? Integer.parseInt(arr[7].trim()) : 0;
					int contactInterval = StringUtils.isNotBlank(arr[8]) ? Integer.parseInt(arr[8].trim()) : 0;
					return new TagContactPacket(sequenceNumber, protocolVersion, beaconLoggerTick, extractTime, battery, guidIndex, count, rssi, contactInterval);
				} catch (NumberFormatException e) {
					LOGGER.error("Exception while mapping csv data into csvData Object. Error : {}, Line : {}", e.getMessage(), line);
				}
			}
		} catch (Exception e) {
			LOGGER.error("Exception while reading csvData", e);
		}
		return null;
	}
}
